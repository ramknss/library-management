...
def updatebook(request,s):
    z=Book.objects.get(id=s)
    if request.method=="POST":
        #z.title=request.POST['a']
        #z.Author=request.POST['y']
        z.Price=request.POST['b']
        z.Edition=request.POST['k']
        z.save()
        return redirect("/")
    return render(request,'book/update_book.html')

def deletebook(request,p):
    n= Book.objects.get(id=p)
    if request.method=="POST":
        n.delete()
        return redirect("/")
    return render(request,'book/deletebook.html',{'z':n})